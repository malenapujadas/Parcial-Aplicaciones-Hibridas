...
export async function getProyectosByUsuario(usuarioId){
    ...
    const proyectos = await db.collection('proyectos').find({ owner: new ObjectId(usuarioId) }).toArray()
    return proyectos
}

export async function getProyectoById(id){
    await client.connect()
    return await db.collection('proyectos').findOne({ _id: new ObjectId(id) })
}

export async function crearProyecto(proyecto){
    await client.connect()
    const result = await db.collection('proyectos').insertOne({ ...proyecto, createdAt: new Date(), updatedAt: new Date() })
    return result.insertedId
}

export async function editarProyecto(id, data, usuarioId){
    await client.connect()
    const res = await db.collection('proyectos').findOne({ _id: new ObjectId(id) })
    if (!res) throw new Error('Proyecto no encontrado')
    if (String(res.owner) !== String(usuarioId)) throw new Error('No autorizado')
    await db.collection('proyectos').updateOne({ _id: new ObjectId(id) }, { $set: { ...data, updatedAt: new Date() } })
    return true
}
